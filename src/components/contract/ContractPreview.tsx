...
export default function ContractPreview({ template, formData, customClauses }: OldContractPreviewProps) {
  if (!template) {
    return (
  ...
  const interpolateClause = (clause: string): string => {
    ...
    return clause.replace(/\{\{(\w+)\}\}/g, (match, key) => formData[key] || `[${key}]`);
  };
  ...
  return (
    <Card className="shadow-lg border-primary/20 h-full flex flex-col">
      <CardHeader className="bg-primary/10">
        <CardTitle className="text-2xl font-headline text-primary-foreground/90">תצוגה מקדימה: {template.name || template.title}</CardTitle>
      </CardHeader>
      <CardContent className="p-6 flex-grow overflow-hidden">
        <ScrollArea className="h-full max-h-[600px] border border-muted p-4 rounded-md bg-background shadow-inner">
          <div className="whitespace-pre-wrap text-sm leading-relaxed font-body">
            <p className="font-bold text-lg mb-4 text-center">{template.name || template.title}</p>
            {contractContent}
            {customClausesText && (
              <>
                {contractContent ? '\n\n' : ''}
