...
const getIconForCategory = (category?: string) => {
  // Made category optional for safety
...
const defaultRentalFields: TemplateField[] = [
  ...
  },
  {
    ...
    label: 'הערות נוספות',
    type: 'textarea',
    placeholder: 'פרטים נוספים או סעיפים מיוחדים...',
    required: false,
  },
];
...
const defaultTemplates: Template[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        id: 'confidentialInformationDescription',
        label: 'תיאור המידע הסודי',
        type: 'textarea',
        required: true,
      },
    ],
...
export const fetchTemplateById = async (
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (localTemplate) {
        console.warn(
          `Template with id ${templateId} not found in Firestore, using local fallback.`
        );
        return {
          ...localTemplate,
          icon: getIconForCategory(localTemplate.category),
        };
      }
      console.warn(
        `Template with id ${templateId} not found in Firestore or local fallbacks.`
      );
      return null;
    }
  } catch (error) {
    ...
    const localTemplateOnError = defaultTemplates.find(
      t => t.id === templateId
    );
    if (localTemplateOnError) {
      console.warn(
        `Error fetching template ${templateId} from Firestore, using local fallback due to error.`
      );
      return {
        ...localTemplateOnError,
        icon: getIconForCategory(localTemplateOnError.category),
      };
    }
    return null;
  }
