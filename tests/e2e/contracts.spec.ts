...
test.describe('Contract Management', () => {
    ...
    test('should handle contract creation flow', async ({ page }) => {
      ...
      if (cardCount > 0) {
        ...
        if (buttonCount > 0) {
          ...
          const hasErrors = await page.getByText(/שגיאה|שדה חובה/i).count() > 0;
  ...
  test.describe('Contract Signing Flow', () => {
    test('should handle contract signing page', async ({ page }) => {
      ...
      const hasContractContent = await page.getByText(/חתימה|חוזה|חתום/i).count() > 0;
      const hasErrorState = await page.getByText(/שגיאה|לא נמצא/i).count() > 0;
