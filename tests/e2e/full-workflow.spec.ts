...
test.describe('Full Contract Workflow', () => {
  ...
  test('should complete full contract creation workflow', async ({ page }) => {
    ...
    if (page.url().includes('/login')) {
      ...
      await expect(page.getByRole('button', { name: /המשך.*גוגל/i })).toBeVisible();
    ...
    if (inputCount > 0) {
      ...
      for (const field of fields) {
        const input = page.getByPlaceholder(field.pattern).or(
          page.getByLabel(field.pattern)
        );
      ...
      // Try to submit
      const submitButton = page.getByRole('button', { name: /שמור|צור|המשך|הבא/i });
      if (await submitButton.count() > 0) {
        ...
        const hasError = await page.getByText(/שגיאה|שדה חובה|נדרש/i).count() > 0;
        const hasSuccess = await page.getByText(/נוצר|נשמר|הצלחה/i).count() > 0;
