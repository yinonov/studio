...
export class AuthHelper {
  ...
  async logout() {
    ...
    const logoutButton = this.page.getByRole('button', { name: /התנתק|יציאה/i });
  ...
  async expectToBeLoggedIn() {
    ...
    await expect(this.page.getByRole('link', { name: /דשבורד|לוח בקרה/i })).toBeVisible();
  }
...
export class ContractHelper {
  ...
  async createRentalContract(data: {
    ...
    // Submit form
    await this.page.getByRole('button', { name: /שמור|צור|המשך/i }).first().click();
    await this.page.waitForLoadState('networkidle');
  }
  ...
  private async fillFieldByLabel(label: string, value: string) {
    ...
    if (await field.isVisible()) {
      ...
      const placeholderField = this.page.getByPlaceholder(new RegExp(label, 'i'));
...
export class TemplateHelper {
  ...
  async expectTemplatesLoaded() {
    ...
    await expect(
      this.page.getByTestId('template-card').or(
        this.page.locator('.loading, [data-testid="loading-skeleton"]')
      )
    ).toBeVisible();
  }
}
