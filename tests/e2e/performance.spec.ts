...
test.describe('Performance Tests', () => {
        ...
        new PerformanceObserver((list) => {
          ...
          entries.forEach((entry) => {
            if (entry.entryType === 'navigation') {
              ...
              metrics.firstPaint = navEntry.loadEventEnd - navEntry.loadEventStart;
            }
          });
  ...
  test('templates page should handle large datasets efficiently', async ({ page }) => {
  ...
  test('should not have excessive bundle size', async ({ page }) => {
    ...
    console.log('JS bundles:', jsBundles.map(b => ({ url: b.url, size: b.size })));
  ...
  test('should be accessible and performant on slow networks', async ({ page, context }) => {
