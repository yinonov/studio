...
export class TestFirebaseHelper {
  ...
  async createTestContract(userId: string, contractData: any) {
    try {
      const contractRef = doc(this.db, 'contracts', `test-contract-${Date.now()}`)
...
export const TestData = {
  ...
  template: {
    rental: {
      ...
      fields: [
        { id: 'party1Name', label: "שם צד א' (משכיר)", type: 'text' as const, required: true },
        { id: 'party2Name', label: "שם צד ב' (שוכר)", type: 'text' as const, required: true },
        { id: 'address', label: 'כתובת הנכס', type: 'text' as const, required: true },
        { id: 'rent', label: 'שכר דירה חודשי', type: 'number' as const, required: true }
      ]
    }
  }
}
